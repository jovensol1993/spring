	...
	<c:if test="${data.upno != 0}">
		<c:set var="subno" value="${subno + 1}" />
					<div class="w3-col pdl30">
						...
						<div class="w3-col m10 l10 pdr10 pdl23" style="height: 7px;">
							<div class="w3-col ${COLOR.list[no * 3 + 2]}" style="width: ${data.per}%; height: 7px;"></div>
						</div>
						<div class="w3-col m2 l2 ${COLOR.textList[no * 3 + 2]} w3-right ptxt">${data.per}%</div>
					</div>
	</c:if>
</c:forEach>
				</div>
