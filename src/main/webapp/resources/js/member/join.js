$(document).ready(function(){
	...
	$('#idck').click(function(){
		...
		$.ajax({
			...
			// data에는 result : "ok" or "no" 가 있음 
			success: function(d){
				// var d = {'result' : 'OK'};
//				result 라는 키값으로 value가 ok라는 문자열이 와야됨
				if(d.result == 'OK'){
					// 사용가능한 아이디인 경우
...
//		var pattern = /^.*(?=^[a-zA-Z0-9#@!$%&-_*]{8,})(?=.*?[a-z])(?=.*?[A-Z])(?=.*?[0-9])(?=.*?[#@!$%&-_*]).*$/;
	...
	*/
		...
		// 비동기 방식 처리
		// 할일
		// form태그 객체를 읽어와서 formData 만들어준다.
		// 비동기 통신에서 form 태그를 전송할 경우
		// 해당 form 태그에 encType 속성이 반드시 기술되어야 한다.
		 
		$('#frm').attr('encType', 'multipart/form-data');
		var el = $('#frm');
		var formData = new FormData($(el)[0]);
		// 이러면 파라미터가 넘어간다(오버로딩된 함수가 실행된다.)
